...
class HubspotObjects {
    ...
    create(config, objects) {
        return __awaiter(this, void 0, void 0, function* () {
            const hubspotClient = new api_client_1.default.Client({ "apiKey": this.hapiKey });
            const properties = {};
    ...
    getByPropertyName(config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (config.objects === "contacts") {
                // @ts-ignore
                yield hubspotClient.crm.contacts.searchApi.doSearch(options).then((res) => JSON.parse(JSON.stringify(res.body.results)));
            }
            else if (config.objects === "companies") {
                // @ts-ignore
                yield hubspotClient.crm.companies.searchApi.doSearch(options).then((res) => JSON.parse(JSON.stringify(res.body.results)));
            }
            else if (config.objects === "deals") {
                // @ts-ignore
                yield hubspotClient.crm.deals.searchApi.doSearch(options).then((res) => JSON.parse(JSON.stringify(res.body.results)));
            }
            else if (config.objects === "tickets") {
                // @ts-ignore
                yield hubspotClient.crm.tickets.searchApi.doSearch(options).then((res) => JSON.parse(JSON.stringify(res.body.results)));
            }
            return apiResponse;
        });
    }
